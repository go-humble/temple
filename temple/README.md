...
Once a template has been added to a group, you can get it directly from the Templates field
of the group:

```go
homeTmpl, found := g.Templates["home"]
if !found {
	// Handle the not found case
}
```

This returns a struct of type [`temple.Template`](http://godoc.org/github.com/go-humble/temple/temple/#Template),
...
```go
indexTmpl, found := g.Templates["index"]
if !found {
	// Handle the not found case
}
...
```go
helloTmpl, found := g.Templates["hello"]
if !found {
	// Handle the not found case
}
