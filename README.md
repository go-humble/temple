...
combintation with other packages in the [Humble Framework](https://github.com/go-humble/humble).
...
to the tepmlate would be `"people/show"`.
...
be treated specially, and they should not overlap with eachother or the src directory with all
...
`template` action. The name of the partial templates is based on its filname and location relative to the
...
You could render it inside of another page like so:
...
templates is based on its filname and location relative to the layouts directory.
[`PartialsPrefix`](http://godoc.org/github.com/go-humble/temple#pkg-variables) is added to the template
...
You could then render a template inside of the layoutby first defining the "content" sub-template and
